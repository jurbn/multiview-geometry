...
n = int(cap.get(cv.CAP_PROP_FRAME_COUNT)/2)
n = 200
...
n = 1
is_open = True
while is_open and n <= 300:
    ret, nxt_frame = cap.read()
    if ret:
        print('Analyzing frame {}'.format(n))
        T_est = extractPose(nxt_frame, X3D, des_arr, sift, K, dist)
        try:
            print(T_est[:, 3][:3])
            drawCamera(fig_triangulation, np.linalg.inv(T_est))
        except Exception:
            print('ERROR')
        n += 1
        cap.set(cv.CAP_PROP_POS_FRAMES, n)

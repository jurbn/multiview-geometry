...
F_12_est = F_21_est.T
pt_arr2 = np.array([pt_arr2[:,i] for i in range(pt_arr2.shape[1]) if inliersMax[i]])
...
x1 = [kp.pt for kp in kp1]
x1 = np.asarray(x1)
x2 = [kp.pt for kp in kp2]
x2 = np.asarray(x2)
X3D = testEstimatedPosesFromFundamental(x1, x2, K, K, RA_21, RB_21, T_21_est.flatten())
...
##### Recuperación de la pose de una tercera cámara
ret, frame2 = cap.read()
kp3, des3 = sift.detectAndCompute(frame1, None)
extractPose(frame2, X3D, des3, sift, K, dist)
